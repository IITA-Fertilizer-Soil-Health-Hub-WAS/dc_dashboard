...
def org_world(django_user_model):
    ...
    uc_a = UseCase.objects.create(code="UC-A", name="Project A", organization=org, country=country)
    uc_b = UseCase.objects.create(code="UC-B", name="Project B", organization=org, country=country)

    member = django_user_model.objects.create_user("m@x.org", "pw", is_active=True, organization=org)
    UseCaseMembership.objects.create(user=member, use_case=uc_a, role=Role.VIEWER)
...
def test_request_existing_membership_is_noop(client, org_world):
    ...
    assert not UseCaseAccessRequest.objects.filter(use_case=org_world["uc_a"]).exists()
...
def test_cannot_request_other_org_project(client, django_user_model, org_world):
    ...
    other_uc = UseCase.objects.create(code="OTHER-UC", name="Other", organization=other_org)
    client.force_login(org_world["member"])  # belongs to 'org'
    resp = client.post(reverse("dashboards:project_request", args=["OTHER-UC"]))
    assert resp.status_code == 403
    assert not UseCaseAccessRequest.objects.filter(use_case=other_uc).exists()
...
def test_coordinator_approves_request_grants_access(client, org_world):
    ...
    req = UseCaseAccessRequest.objects.create(user=member, use_case=uc_b)
    ...
    assert UseCaseMembership.objects.filter(
        user=member, use_case=uc_b, role=Role.ENUMERATOR
    ).exists()
    # The requester now sees the project.
    assert visible_use_cases(member).filter(pk=uc_b.pk).exists()
...
def test_coordinator_declines_request(client, org_world):
    ...
    assert not UseCaseMembership.objects.filter(user=member, use_case=uc_b).exists()
...
def test_request_outside_authority_not_listed(client, django_user_model, org_world):
    ...
    UseCaseAccessRequest.objects.create(user=member, use_case=uc_b)
    ...
    UseCaseMembership.objects.create(user=outsider, use_case=org_world["uc_a"],
