...
def test_match_by_phone(django_user_model, use_case):
    user = django_user_model.objects.create_user(
        "a@x.org", "pw", full_name="Aline Uwase", phone="+250788000000"
    )
    en = Enumerator.objects.create(
        use_case=use_case, enid="EN1", first_name="Different", surname="Name",
...
def test_match_by_name_when_no_phone(django_user_model, use_case):
    user = django_user_model.objects.create_user("b@x.org", "pw", full_name="John Doe")
    en = Enumerator.objects.create(
        use_case=use_case, enid="EN2", first_name="john", surname="DOE"
    )
    report = link_enumerators(apply=True)
    en.refresh_from_db()
    assert en.user == user
...
def test_dry_run_does_not_persist(django_user_model, use_case):
    django_user_model.objects.create_user("c@x.org", "pw", phone="0788111222")
    en = Enumerator.objects.create(use_case=use_case, enid="EN3", phone="0788111222")
...
def test_ambiguous_match_not_linked(django_user_model, use_case):
    ...
    en = Enumerator.objects.create(use_case=use_case, enid="EN4", phone="0788333444")
    report = link_enumerators(apply=True)
    en.refresh_from_db()
...
def test_already_linked_skipped_without_overwrite(django_user_model, use_case):
    ...
    en = Enumerator.objects.create(use_case=use_case, enid="EN5", phone="0788555666", user=u1)
...
def test_unmatched_when_no_signal(use_case):
    Enumerator.objects.create(use_case=use_case, enid="EN6")  # no phone, no name
    report = link_enumerators(apply=True)
    assert report.unmatched == 1
    assert report.matched == 0


def test_scope_to_one_use_case(django_user_model):
    uc1 = UseCase.objects.create(code="UC1", name="UC1")
    uc2 = UseCase.objects.create(code="UC2", name="UC2")
    django_user_model.objects.create_user("f@x.org", "pw", phone="0788777888")
    en1 = Enumerator.objects.create(use_case=uc1, enid="E1", phone="0788777888")
    en2 = Enumerator.objects.create(use_case=uc2, enid="E2", phone="0788777888")

    link_enumerators(apply=True, use_case=uc1)
    en1.refresh_from_db()
    en2.refresh_from_db()
...
def test_link_then_sync_populates_collected_by(django_user_model, use_case):
    ...
    Enumerator.objects.create(use_case=use_case, enid="EN7", phone="0788999000")
    link_enumerators(apply=True)

    form = FormDefinition.objects.create(
        use_case=use_case, ona_form_id=42, role=FormDefinition.Role.VALIDATION
    )
    ...
    sync_use_case(use_case, client=Fake())
    sub = Submission.objects.get(use_case=use_case, ona_uuid="uu")
