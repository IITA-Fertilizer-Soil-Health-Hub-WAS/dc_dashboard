...
def endorser(world, django_user_model):
    ...
    Membership.objects.create(user=user, project=world["uc"], role=Role.TRIAL_COORDINATOR)
    return user
...
def validator(world, django_user_model):
    ...
    Membership.objects.create(user=user, project=world["uc"], role=Role.REGIONAL_COORDINATOR)
    return user
...
def test_qc_queue_forbidden_without_validator_right(client, world, django_user_model):
    ...
    Membership.objects.create(user=plain, project=world["uc"], role=Role.ENUMERATOR)
