...
def test_regional_coordinator_spans_region(django_user_model, geo):
    ...
    UseCaseMembership.objects.create(user=rc, region=geo["ea"], role=Role.REGIONAL_COORDINATOR)
...
def test_view_rejects_out_of_scope_grant(client, django_user_model, geo):
    ...
    assert not UseCaseMembership.objects.filter(user=target).exists()
...
def test_view_revoke_only_within_authority(client, django_user_model, geo):
    ...
    assert resp.status_code == 403
    assert UseCaseMembership.objects.filter(pk=out.pk).exists()  # not revoked
