...
def test_country_coordinator_can_grant_within_country(django_user_model, geo):
    ...
    ProjectMembership.objects.create(user=cc, country=geo["rwanda"], role=Role.COUNTRY_COORDINATOR)
...
def test_regional_coordinator_spans_region(django_user_model, geo):
    ...
    ProjectMembership.objects.create(user=rc, region=geo["ea"], role=Role.REGIONAL_COORDINATOR)
...
def test_trial_coordinator_limited_to_one_project(django_user_model, geo):
    ...
    ProjectMembership.objects.create(user=tc, project=geo["uc_rw"], role=Role.TRIAL_COORDINATOR)
...
def test_non_coordinator_cannot_manage_access(django_user_model, geo):
    ...
    ProjectMembership.objects.create(user=viewer, project=geo["uc_rw"], role=Role.VIEWER)
...
def test_manageable_memberships_scoped(django_user_model, geo):
    ...
    in_scope = ProjectMembership.objects.create(user=other, project=geo["uc_rw"], role=Role.VIEWER)
    out_scope = ProjectMembership.objects.create(user=other, project=geo["uc_ke"], role=Role.VIEWER)
...
def test_view_rejects_out_of_scope_grant(client, django_user_model, geo):
    ...
    assert not ProjectMembership.objects.filter(user=target).exists()
...
def test_view_revoke_only_within_authority(client, django_user_model, geo):
    ...
    out = ProjectMembership.objects.create(user=victim, project=geo["uc_ke"], role=Role.VIEWER)
    client.force_login(cc)

    resp = client.post(reverse("dashboards:team_revoke"), {"membership": str(out.pk)})
    assert resp.status_code == 403
    assert ProjectMembership.objects.filter(pk=out.pk).exists()  # not revoked
