...
def qc(django_user_model, uc):
    ...
    Membership.objects.create(user=user, project=uc, role=Role.COUNTRY_COORDINATOR)
    return user
...
def coordinator(django_user_model, uc):
    ...
    Membership.objects.create(user=user, project=uc, role=Role.TRIAL_COORDINATOR)
    return user
...
def regional(django_user_model, uc):
    ...
    Membership.objects.create(user=user, project=uc, role=Role.REGIONAL_COORDINATOR)
    return user
