...
def _make_worsening(world, enid):
    enum = Enumerator.objects.create(use_case=world["uc"], enid=enid)
...
def _rule(world):
    return AlertRule.objects.create(
        use_case=world["uc"], name="Quality degrading",
...
def test_alert_fires_and_names_the_enumerator(world):
    ...
    event = evaluate_rule_for_use_case(_rule(world), world["uc"])
    assert event is not None
    assert "EN-BAD" in event.message and "0%→100%" in event.message


def test_no_alert_when_nobody_worsening(world):
    enum = Enumerator.objects.create(use_case=world["uc"], enid="EN-OK")
...
def test_low_volume_worsening_is_ignored(world):
    enum = Enumerator.objects.create(use_case=world["uc"], enid="EN-TINY")
