...
def test_decline_records_rejection_reason(world):
    ...
    reason = RejectionReason.objects.create(use_case=uc, code="too-far", label="Too far from plot")
    sub = Submission.objects.create(use_case=uc, form=form, ona_uuid="d1", content_hash="h")
...
def test_review_screen_offers_reasons_and_captures(client, world):
    uc, form, coord = world["uc"], world["form"], world["coord"]
    reason = RejectionReason.objects.create(use_case=uc, code="security", label="Security threat")
    RejectionReason.objects.create(code="global-x", label="Global reason")  # global (no use_case)
    sub = Submission.objects.create(use_case=uc, form=form, ona_uuid="d2", content_hash="h")
...
def test_job_progress_qc_and_closure(world, django_user_model):
    uc, form, coord = world["uc"], world["form"], world["coord"]
    job = Job.objects.create(use_case=uc, name="J1")
    u1 = CollectionUnit.objects.create(use_case=uc, code="U1")
    u2 = CollectionUnit.objects.create(use_case=uc, code="U2")
    UnitAssignment.objects.create(job=job, unit=u1)
    UnitAssignment.objects.create(job=job, unit=u2)
    s1 = Submission.objects.create(use_case=uc, form=form, ona_uuid="s1", content_hash="h", collection_unit=u1)
    Submission.objects.create(use_case=uc, form=form, ona_uuid="s2", content_hash="h", collection_unit=u2)
...
def test_submission_is_corrected(world):
    ...
    sub = Submission.objects.create(use_case=uc, form=form, ona_uuid="c1", content_hash="h")
