...
from apps.kpi.alerts import evaluate_rule_for_use_case, run_alerts
...
def _seed_submissions(uc, n, on=None):
    form = FormDefinition.objects.create(use_case=uc, ona_form_id=7,
...
def test_threshold_not_breached_no_event(org):
    uc = UseCase.objects.create(code="PB", name="PB", organization=org)
    _seed_submissions(uc, 10)  # above threshold
    rule = AlertRule.objects.create(
        use_case=uc, name="Low volume", metric="daily_submissions",
        comparator=AlertRule.Comparator.LT, threshold=5,
    )
    assert evaluate_rule_for_use_case(rule, uc) is None


def test_idempotent_same_day(org):
    uc = UseCase.objects.create(code="PC", name="PC", organization=org)
    _seed_submissions(uc, 0)
    rule = AlertRule.objects.create(
        use_case=uc, name="Silent", metric="daily_submissions",
        comparator=AlertRule.Comparator.LTE, threshold=0,
    )
    assert evaluate_rule_for_use_case(rule, uc) is not None
...
def test_open_flags_metric(org):
    uc = UseCase.objects.create(code="PD", name="PD", organization=org)
    form, en = _seed_submissions(uc, 2)
    vr = ValidationRule.objects.create(use_case=uc, code="r",
                                       rule_type=ValidationRule.RuleType.REGEX_ID,
                                       severity=ValidationRule.Severity.ERROR)
    sub = Submission.objects.filter(use_case=uc).first()
    ValidationFlag.objects.create(submission=sub, rule=vr, message="x",
                                  severity=ValidationRule.Severity.ERROR,
                                  status=ValidationFlag.Status.OPEN)
    rule = AlertRule.objects.create(
        use_case=uc, name="Too many issues", metric="open_flags",
...
def test_run_alerts_emails_watchers(org):
    uc = UseCase.objects.create(code="PE", name="PE", organization=org)
    _seed_submissions(uc, 0)
...
def test_platform_wide_rule_evaluates_all_projects(org):
    a = UseCase.objects.create(code="PF", name="PF", organization=org)
    b = UseCase.objects.create(code="PG", name="PG", organization=org)
    _seed_submissions(a, 0)
    _seed_submissions(b, 0)
    AlertRule.objects.create(  # use_case null = platform-wide
...
def test_alerts_view_scoped(client, django_user_model, org):
    uc = UseCase.objects.create(code="PH", name="PH", organization=org)
    other = UseCase.objects.create(code="PZ", name="PZ", organization=org)
    rule_a = AlertRule.objects.create(use_case=uc, name="A", metric="open_flags")
    rule_z = AlertRule.objects.create(use_case=other, name="Z", metric="open_flags")
    AlertEvent.objects.create(rule=rule_a, use_case=uc, severity="WARNING",
                              message="visible alert", observed_value=0)
    AlertEvent.objects.create(rule=rule_z, use_case=other, severity="WARNING",
