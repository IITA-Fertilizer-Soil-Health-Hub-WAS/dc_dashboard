...
def test_project_quality_trend_detects_systemic_slide(world):
    ...
    for i in range(4):
        s = Submission.objects.create(
            project=world["uc"], form=world["form"], ona_uuid=f"old{i}", content_hash=f"old{i}",
            event_date=date.today() - timedelta(days=70 + i))
        assert s  # clean
    for i in range(4):
        s = Submission.objects.create(
            project=world["uc"], form=world["form"], ona_uuid=f"new{i}", content_hash=f"new{i}",
...
def test_enumerator_detail_view_renders(client, world, django_user_model):
    ...
    UseCaseMembership.objects.create(user=coord, project=world["uc"], role=Role.TRIAL_COORDINATOR)
