...
def test_coordinator_scoped_to_their_project(users, projects):
    ...
    ProjectMembership.objects.create(user=coord, project=rwanda, role=Role.TRIAL_COORDINATOR)
...
def test_country_coordinator_is_gate1(users, projects):
    ...
    rwanda, _ = projects
    ProjectMembership.objects.create(user=person, project=rwanda, role=Role.COUNTRY_COORDINATOR)
    ProjectMembership.objects.create(user=regional, project=rwanda, role=Role.REGIONAL_COORDINATOR)
...
def test_regional_coordinator_is_gate2(users, projects):
    ...
    _, _, _, person = users
    rwanda, _ = projects
    ProjectMembership.objects.create(user=person, project=rwanda, role=Role.REGIONAL_COORDINATOR)
...
def test_viewer_is_read_only(users, projects):
    _, _, _, viewer = users
    rwanda, _ = projects
    ProjectMembership.objects.create(user=viewer, project=rwanda, role=Role.VIEWER)
...
def test_inactive_user_denied(django_user_model, projects):
    ...
    ProjectMembership.objects.create(user=pending, project=rwanda, role=Role.TRIAL_COORDINATOR)
