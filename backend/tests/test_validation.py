...
def test_clean_data_no_id_flags(use_case):
    run_for_use_case(use_case)
...
def test_bad_enid_is_flagged_and_review_flagged(use_case):
    ...
    stats = run_for_use_case(use_case)
...
def test_flag_auto_resolves_when_fixed(use_case):
    s = Submission.objects.get(ona_uuid="uuid-aaa")
...
def test_event_sequence_flagged(use_case):
    # Household has Event1 + Event2 already; delete Event1 to create a gap.
    Submission.objects.filter(use_case=use_case, event_key="Event1").delete()
    run_for_use_case(use_case)
    assert ValidationFlag.objects.filter(rule__code="event_sequence").exists()


def test_idempotent_no_duplicate_flags(use_case):
    s = Submission.objects.get(ona_uuid="uuid-aaa")
    SubmissionValue.objects.filter(submission=s, field_key="ENID").update(current_value="BADID")
    run_for_use_case(use_case)
    run_for_use_case(use_case)
