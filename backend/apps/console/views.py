...
class EnumeratorLinkView(ManageMixin, View):
    ...
    def _unlinked_and_accounts(self, request):
        """Enumerators with no account yet (scoped) + the accounts a coordinator may
        link them to (active users in the same institutions), for the manual picker."""
        from apps.accounts.models import User
        from apps.projects.models import Project
        from apps.submissions.models import Enumerator

        scope = self._scope(request)
        enums = Enumerator.objects.filter(is_test=False, user__isnull=True).select_related("project")
        if scope is not None:
            enums = enums.filter(project_id__in=scope)
        enums = list(enums.order_by("project__code", "enid")[:500])

        org_ids = set(
            Project.objects.filter(id__in={e.project_id for e in enums})
            .values_list("organization_id", flat=True)
        )
        accounts = User.objects.filter(is_active=True)
        if org_ids:
            accounts = accounts.filter(organization_id__in=org_ids)
        return enums, list(accounts.order_by("email")[:500])

    def get(self, request):
        from apps.submissions.linking import link_enumerators

        report = link_enumerators(apply=False, projects=self._scope(request))
        unlinked, accounts = self._unlinked_and_accounts(request)
    ...
    def _link_one(self, request):
        """Manually link one enumerator to one account (resolves ambiguous/unmatched
        rows the auto-matcher can't). Tenant-guarded to the project's institution."""
        from apps.accounts.models import User
        from apps.submissions.models import Enumerator

        scope = self._scope(request)
        enums = Enumerator.objects.select_related("project")
        if scope is not None:
            enums = enums.filter(project_id__in=scope)
        enum = enums.filter(pk=request.POST.get("enumerator")).first()
        user = User.objects.filter(pk=request.POST.get("user"), is_active=True).first()
        if enum is None or user is None:
            messages.error(request, "Pick both an enumerator and an account.")
            return
        org = enum.project.organization_id
        if org and user.organization_id and user.organization_id != org:
            messages.error(request, "That account belongs to another institution.")
            return
        enum.user = user
        enum.save(update_fields=["user", "updated_at"])
        messages.success(
            request, f"Linked {enum.enid} → {user.user_id or user.email}. "
            "collected_by populates on the next sync."
        )

    def post(self, request):
        from apps.submissions.linking import link_enumerators

        if request.POST.get("action") == "link_one":
            self._link_one(request)
        ...
        if report.ambiguous:
            messages.warning(
                ...
                "account — use 'Link manually' below to resolve them."
            )
        return redirect("console:link_enumerators")
