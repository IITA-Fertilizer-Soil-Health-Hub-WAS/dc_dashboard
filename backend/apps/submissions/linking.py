...
class LinkReport:
    ...
    def actionable(self) -> list[Proposal]:
        ...
        return sorted(
            ...
            key=lambda p: (order[p.status], p.project, p.enid),
        )
...
def link_enumerators(
    *,
    ...
    re-evaluates enumerators that already have a linked account. ``project``
    limits the scope to one project; ``projects`` (an iterable of Project or
    ...
    qs = Enumerator.objects.select_related("project")
    if project is not None:
        qs = qs.filter(project=project)
    if projects is not None:
        qs = qs.filter(project__in=list(projects))

    for enum in qs:
        base = dict(enumerator_pk=str(enum.pk), enid=enum.enid, project=enum.project.code)
